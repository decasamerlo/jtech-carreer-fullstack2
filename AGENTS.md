...
- **UI Framework**: Vuetify 4 + @mdi/font (Material Design 3)
...
- **vite-plugin-vuetify**: Vite plugin for component/directive auto-import
...
- **Test files**: in `src/**/__tests__/`, covering stores, services, most components, the router guard, and the views (`LoginView`, `RegisterView`, `HomeView`, `AboutView`, `ListsView`). `TaskListSidebar.vue` has a dedicated spec for its list-rendering logic (its dialogs are tested individually).
